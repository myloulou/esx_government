...
AddEventHandler('esx_statejob:publicTeleports', function(position)
...
function OpenCloakroomMenu()
  ...
  if PlayerData.job.grade_name == 'recrue' then
    table.insert(elements, {label = _U('state_wear'), value = 'cadet_wear'})
  end

  if PlayerData.job.grade_name == 'garde' then
    table.insert(elements, {label = _U('state_wear'), value = 'sergeant_wear'})
  end

  if PlayerData.job.grade_name == 'vicepresident' then
    table.insert(elements, {label = _U('state_wear'), value = 'lieutenant_wear'})
  end

  if PlayerData.job.grade_name == 'boss' then
    table.insert(elements, {label = _U('state_wear'), value = 'commandant_wear'})
  end
  ...
  if Config.EnableArmoryManagement then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        TriggerServerEvent('esx_statejobjob:giveWeapon', weapon,  1000)
      end,
      function(data, menu)
...
function OpenVehicleSpawnerMenu(station, partNum)
  ...
  if Config.EnableSocietyOwnedVehicles then
    ...
    table.insert(elements, { label = 'Sortie VIP', value = 'stretch' })
    table.insert(elements, { label = 'Escorte Ville', value = 'kuruma2' })
    table.insert(elements, { label = 'Escorte Montagne', value = 'baller6' })
    table.insert(elements, { label = 'Transport Hélico', value = 'volatus' })
    table.insert(elements, { label = 'Transport Hélico', value = 'swift' })
    table.insert(elements, { label = 'Détente', value = 'faggio3' })
    table.insert(elements, { label = 'Sortie Ville/Montagne', value = 'dubsta2' })
          ...
          function(data2, menu2)
            ...
            if distance ~= -1 and distance <= 3.0 then
              ...
              if data2.current.value == 'handcuff' then
                TriggerServerEvent('esx_statejob:handcuff', GetPlayerServerId(player))
              end

              if data2.current.value == 'drag' then
                TriggerServerEvent('esx_statejob:drag', GetPlayerServerId(player))
              end

              if data2.current.value == 'put_in_vehicle' then
                TriggerServerEvent('esx_statejob:putInVehicle', GetPlayerServerId(player))
              end

              if data2.current.value == 'out_the_vehicle' then
                  TriggerServerEvent('esx_statejob:OutVehicle', GetPlayerServerId(player))
              end
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if Config.EnablePlayerManagement then
          TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_state', _U('fine_total') .. label, amount)
        else
  ...
  ESX.TriggerServerCallback('esx_statejob:getArmoryWeapons', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_statejob:buy', function(hasEnoughMoney)
  ...
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'state' then
      ...
      for k,v in pairs(Config.StateStations) do
        ...
        if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'state' and PlayerData.job.grade_name == 'boss' then
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
		...
		TriggerEvent('esx_statejob:publicTeleports', position)
  ...
  while true do
    ...
    if closestDistance ~= -1 and closestDistance <= 3.0 then
      ...
      if LastEntity ~= closestEntity then
        TriggerEvent('esx_statejob:hasEnteredEntityZone', closestEntity)
      ...
      if LastEntity ~= nil then
        TriggerEvent('esx_statejob:hasExitedEntityZone', LastEntity)
        LastEntity = nil
      end
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'state' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'state_actions') and (GetGameTimer() - GUI.Time) > 150 then
